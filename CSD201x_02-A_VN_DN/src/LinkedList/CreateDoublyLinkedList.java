...
public class CreateDoublyLinkedList {
    ...
    public void addNewNode(String data) {
        ...
        // Execute when the doubly linked list is not empty
            ...
            // The newly created node will be the last node, so now tail's next will point
            // to that newly created node
            tail.next = newNode;
            // The tail is pointing to the second last node so the newly created node's prev
            // will point to tail
            newNode.prev = tail;
            // The newly created node will become new tail because it is last node in the
            // doubly linked list
            tail = newNode;
    ...
    public void showData() {
        ...
        while (current != null) {
            // Print tha data on that particular node and then increment the pointer for
            // indicating next node
