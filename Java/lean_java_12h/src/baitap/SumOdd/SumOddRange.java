...
public class SumOddRange {
     ...
     * 
     * ? Write a second method called sumOdd that has 2 int parameters start and
     * ? end,
    ...
    public static boolean isOdd(int number) {
        if (number > 0) { // kiểm tra "number" có lớn hơn 0
            if (number % 2 == 1) { // kiểm tra xem "number" có phải là số lẻ không
                return true;
            } else {
                return false;
            }
    ...
    public static int sumOdd(int start, int end) {
        int sum = 0;
        if (start > 0 && end > 0 && end >= start) { // kiểm tra xem "start" và "end"
            // có lớn hơn 0 không và start < end
            for (int i = start; i <= end; i++) { // ! khởi tạo i bằng với vị trí "start" và i <= "end"
                if (isOdd(i)) {// ! gọi hàm kiểm tra số lẻ (isOdd)
                    sum += i;
                }
            }
            return sum;
